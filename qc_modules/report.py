...
_END_REASON_INFO = (
    ...
    '<tbody>'
    '<tr><td><b>signal_positive</b></td><td>'
    'Normal, complete read. The DNA strand finished translocating through the pore and the '
    'ionic current returned to an open-pore level. The most desirable outcome; a healthy run '
    'should have the large majority of reads ending this way.'
    '</td></tr>'
    '<tr><td><b>unblock_mux_change</b></td><td>'
    'Read terminated by either (a) a routine mux scan — MinKNOW periodically tests all pores '
    'in each channel group and selects the best performer, interrupting any read in progress — '
    'or (b) an active unblock, where MinKNOW applies a reverse voltage to eject a strand that '
    'has stalled or is blocking the pore. Both events produce this label in modern '
    'MinKNOW/Dorado. A moderate rate (~5&ndash;10%) is normal; the mux scan contribution alone '
    'is typically 1&ndash;3%. Routine pore clearing throughout the run accounts for most of the '
    'remainder. A very high rate may indicate poor DNA quality, excess short fragments, or '
    'active adaptive sampling (ReadUntil).'
    '</td></tr>'
    '<tr><td><b>data_service_unblock_mux_change</b></td><td>'
    'Unblock triggered specifically by the basecalling / data service layer rather than by '
    'MinKNOW itself. Seen when adaptive sampling (e.g. ReadFish, Dorado real-time enrichment) '
    'rejects a molecule mid-read. If this category is large, adaptive sampling was active '
    'and rejecting a significant fraction of molecules.'
    '</td></tr>'
    '<tr><td><b>signal_negative</b></td><td>'
    'Read ended due to an anomalous negative or very low ionic current signal. Can indicate a '
    'stalled strand, a molecule reversing direction through the pore, or a pore blockage '
    'producing an unusual electrical signature. Typically a small fraction of reads.'
    '</td></tr>'
    '<tr><td><b>mux_change</b></td><td>'
    'Rare in modern runs. Seen in older MinKNOW versions as the label for mux-scan-terminated '
    'reads; in current software this role is filled by unblock_mux_change. When present at '
    'very low counts it likely reflects a brief period of an older MinKNOW version or a '
    'channel-level mux switch that did not involve an active unblock voltage.'
    '</td></tr>'
    '<tr><td><b>paused</b></td><td>'
    'The run was paused (by the user or automatically by MinKNOW) while this read was in '
    'progress. The read is terminated at the point of the pause.'
    '</td></tr>'
    '<tr><td><b>user_regulated_pause</b></td><td>'
    'Equivalent to paused; the label used in some MinKNOW versions when the pause was '
    'user-initiated via the MinKNOW UI.'
    '</td></tr>'
    '<tr><td><b>analysis_config_change</b></td><td>'
    'MinKNOW applied a change to the analysis configuration (e.g. basecalling model, '
    'experiment settings) mid-read. The current read is ended so the new configuration '
    'takes effect cleanly.'
    '</td></tr>'
    '<tr><td><b>forced</b></td><td>'
    'MinKNOW forcibly terminated the read, typically because the channel crossed a pore '
    'quality or signal threshold that made continued sequencing unproductive.'
    '</td></tr>'
    '<tr><td><b>unknown</b></td><td>'
    'End reason not recorded or not recognised. Common in older Guppy or Albacore files '
    'that predate the end_reason field.'
    '</td></tr>'
    '</tbody></table>'
)
